...
Mock figures in `docs/figures/claim1/`. Run `python docs/mock_claim1_figures.py` to regenerate.

### Fig C1 — CCE Score vs Oracle Q* Consequence (3 training stages)

![CCE vs Oracle scatter at 3 stages](figures/claim1/fig_c1_scatter_stages.png)

Three-panel scatter plot — one panel per training stage (untrained, mid-training, fully
trained). Each point is one non-terminal state of the 8×8 grid. X-axis: oracle Q*
suboptimality gap (ground truth importance). Y-axis: CCE Wasserstein score. Points
colored by proximity to holes (red = adjacent, orange = 2 steps, blue = safe).
Spearman ρ annotated per panel.

**The money shot.** Shows the correlation emerging from noise as policy quality improves.
Expected: ρ ≈ 0.0–0.1 (untrained) → ρ ≈ 0.5 (mid) → ρ ≈ 0.7+ (trained).

---

### Fig C2 — 8×8 Importance Heatmaps

![8x8 importance heatmaps](figures/claim1/fig_c2_grid_heatmaps.png)

Three side-by-side 8×8 grids: oracle importance (ground truth) | CCE untrained |
CCE fully trained. Color scale YlOrRd (yellow = low, red = high). Holes shown as dark
✕, goal as G. The trained CCE heatmap should visually resemble the oracle — hot cells
adjacent to holes, cool cells in safe regions.

**Most intuitive figure.** A reader immediately sees "the algorithm found what the oracle
finds." Untrained panel shows no structure (noise). Trained panel matches oracle.

---

### Fig C3 — Claim 1 Metrics vs Training Progress

![Metric progression over training](figures/claim1/fig_c3_metric_progression.png)

Three panels showing all three metrics over training checkpoints (0–15k episodes),
with 3 seed traces + mean + 95% CI band. Vertical dashed lines mark the untrained,
mid-training, and fully trained snapshots from C1.

- **Spearman ρ**: rises from ~0 to ~0.74 (green)
- **Precision@10%**: rises from 0.10 (random baseline) to ~0.62 (blue)
- **Sampling KL**: falls from ~1.8 to ~0.08 (red)

Shows the monotonic improvement across all three metrics simultaneously.

---

### Fig C4 — Precision@K vs Random Baseline

![Precision at K bar chart](figures/claim1/fig_c4_precision_at_k.png)

Bar chart for the fully trained policy across 3 seeds. Green bars = CCE at K = 5%,
10%, 20%. Gray bars = random baseline (= K by definition). Annotated with lift ratio
(e.g. "13.7× random"). Error bars from seed variance.

Shows CCE beats chance at every threshold, with the strongest separation at K=5%
(hardest, most meaningful).
