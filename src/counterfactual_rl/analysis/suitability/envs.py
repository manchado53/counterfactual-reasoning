...
the new slip_prob), generalizing analysis/diagnostics/value_iteration.py:compute_qstar.
"""
...
def outcome_probs(env) -> np.ndarray:
    ...
    slip_prob set → env.outcome_probs; legacy slippery/deterministic → uniform (correct
...
def qstar_spread_exact(env, gamma: float, tol: float = 1e-12, max_iter: int = 100_000) -> np.ndarray:
    ...
    outcome probabilities, so it is correct for is_slippery True/False AND any slip_prob."""
