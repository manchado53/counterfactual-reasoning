"""
DQN and DQN+PER agents for DoorKey.
...
class _MetricsLogger:
    ...
    def __init__(self, config: dict, n_episodes: int, eval_interval, eval_episodes: int):
        ...
        self._file.write(f"# DoorKey DQN — {datetime.now()}\n")
        self._file.write(
            f"# Layout: {config['layout_name']}  Slip: {config.get('slip_prob', 0.0)}"
...
class DoorKeyDQN:
    """
    DQN (uniform or PER) for single-agent DoorKey.

    Observation encoding: one-hot(state_index) → (n_states,) MLP input.
    No action masking — all 7 DoorKey actions always valid.
    """
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.env = DoorKeyEnv(
            ...
            slip_prob=self.config.get('slip_prob', 0.0),
        )
    ...
    def learn(self, n_episodes: Optional[int] = None, verbose: bool = True) -> 'DoorKeyDQN':
        ...
        if verbose:
            ...
            print(f"Training {alg} on DoorKey-{self.config['layout_name']}")
