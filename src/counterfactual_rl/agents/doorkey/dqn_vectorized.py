"""
Vectorized DQN for DoorKey using lax.scan + vmap over n_envs parallel environments.
...
from .dqn import DoorKeyDQN, _MetricsLogger


class DoorKeyDQNVectorized(DoorKeyDQN):
    """
    ...
    def _build_collect_fn(self):
        ...
        max_ep_steps = self.config.get('max_episode_steps', 100)
        ...
        def _collect_single_env(params, epsilon, init_state, step_keys):
            ...
            def _step(carry, step_key):
                ...
                # Environment step
                _, next_state, reward, env_done, _ = env.step(k_env, state, action)
                new_cum = cum_return + reward
                new_len = ep_length + jnp.int32(1)

                # DoorKey's only terminal is the goal, so cap episode length
                # (time-limit truncation) — guarantees episodes end and epsilon decays.
                truncated = new_len >= max_ep_steps
                episode_over = jnp.logical_or(env_done, truncated)

                # Emit episode stats only at an episode boundary (goal OR truncation)
                ep_return_out = jnp.where(episode_over, new_cum, jnp.float32(0.0))
                ep_length_out = jnp.where(episode_over, new_len, jnp.int32(0))
                ep_ended_out = episode_over

                # Buffer stores the REAL terminal flag (env_done) so bootstrapping stays
                # correct on truncated steps; auto-reset uses episode_over.
                return (next_state, episode_over, new_cum, new_len), (
                    state, action, reward, next_state, env_done,
    ...
    def learn(self, n_episodes: Optional[int] = None, verbose: bool = True) -> 'DoorKeyDQNVectorized':
        ...
        if verbose:
            ...
            print(f"Training {alg} on DoorKey-{self.config['layout_name']} [VECTORIZED]")
