...
Inherits CCE scoring, rollout function, and buffer from DoorKeyConsequenceDQN.
...
class DoorKeyConsequenceDQNVectorized(DoorKeyConsequenceDQN):
    """
    CCE-DQN with vectorized collection.

    Builds the same lax.scan + vmap collect function as DoorKeyDQNVectorized
    ...
    def _build_collect_fn(self):
        ...
        DoorKeyDQNVectorized._build_collect_fn(self)

    def _build_eval_fn(self):
        DoorKeyDQNVectorized._build_eval_fn(self)

    def evaluate(self, n_episodes: int = 100) -> dict:
        return DoorKeyDQNVectorized.evaluate(self, n_episodes)
    ...
    def learn(self, n_episodes: Optional[int] = None, verbose: bool = True) -> 'DoorKeyConsequenceDQNVectorized':
        ...
        if verbose:
            ...
            print(f"Training Consequence-DQN on DoorKey-{self.config['layout_name']} [VECTORIZED]")
