"""
Consequence-weighted DQN (Algorithm 2) for DoorKey.
...
# action set is built per-instance from env.n_actions (DoorKey has 7 actions) —
# see self._all_actions in __init__.


class DoorKeyConsequenceDQN(DoorKeyDQN):
    """
    Consequence-weighted DQN for DoorKey (Algorithm 2).
    ...
    def _score_buffer_transitions(self):
        ...
        keys_flat = jax.random.split(subkey, B * self.n_actions * N)
        keys_array = keys_flat.reshape(B, self.n_actions, N, 2)
        ...
        with timer('update.scoring.metrics', episode=ep):
            ...
            for i in range(B):
                ...
                return_distributions = {(a,): returns_np[i, a] for a in range(self.n_actions)}
    ...
    def learn(self, n_episodes: Optional[int] = None, verbose: bool = True) -> 'DoorKeyConsequenceDQN':
        ...
        if verbose:
            ...
            mu = self.config.get('mu', 0.5)
            print(f"Training Consequence-DQN on DoorKey-{self.config['layout_name']}")
            print(f"  Slip: {self.config.get('slip_prob', 0.0)}  |  n_states: {self.n_states}")
        ...
        for episode in pbar:
            ...
            while not done and ep_steps < self.config.get('max_episode_steps', 100):
